...
//                 <SelectTrigger id="rifa-select" className="w-full">
...
//               {/* Mostrar título completo debajo si está seleccionado */}
//               {/* {rifaSeleccionada && (
//                 <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
//                   <p className="text-sm text-blue-900">
//                     <span className="font-semibold">Rifa seleccionada:</span>
//                     <br />
//                     <span className="break-words">
//                       {rifasActivas.find(r => r.id === rifaSeleccionada)?.titulo}
//                     </span>
//                   </p>
//                 </div>
//               )} */}
...
import { toast } from "sonner"
import SelectorPaquetes from "../../../agency/pages/ventas/components/SelectorPaquetes"
import { TipoVentaSelector } from "../../../agency/pages/ventas/components/TipoVentaSelector"  
...
export const AgencyVentasPage = () => {
  ...
  const [rifaConfirmada, setRifaConfirmada] = useState<string>("")  
  ...
  const handleRifaChange = (rifaId: string) => {
    setRifaSeleccionada(rifaId)
    setDialogOpen(true)  //  Abrir dialog al seleccionar rifa
  }

  const handleTipoVentaConfirm = () => {
    setRifaConfirmada(rifaSeleccionada)  // Confirmar rifa
    setDialogOpen(false)  // Cerrar dialog
    toast.success('Tipo de venta seleccionado', {
      description: `Has seleccionado venta por ${tipoVenta === 'email' ? 'Email' : tipoVenta === 'whatsapp' ? 'WhatsApp' : 'SMS'}`,
      duration: 2000
    })
  }
  ...
  const handleDialogClose = () => {
    setDialogOpen(false)
    setRifaSeleccionada("")  //  Limpiar selección si cierra sin confirmar
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ NUEVO: Dialog para seleccionar tipo de venta */}
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl">Seleccionar Tipo de Notificación</DialogTitle>
            <DialogDescription>
              Elige cómo deseas notificar al cliente sobre la compra
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              onClick={handleTipoVentaConfirm}
              className={
                tipoVenta === 'whatsapp' 
      ...
      {/* Componente de ventas - Solo se muestra después de confirmar */}
      {rifaConfirmada && (
        <SelectorPaquetes 
          key={`selector-${rifaConfirmada}-${refreshKey}`}
          rifaId={rifaConfirmada}
          agenciaId={agenciaId}
          refreshKey={refreshKey}
          tipoVentaInicial={tipoVenta}  // ← Pasar tipo de venta inicial
        />
      )}
    </div>
  )
