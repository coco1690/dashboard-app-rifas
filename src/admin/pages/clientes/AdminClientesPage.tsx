...
export const AdminClientesPage = () => {
  const navigate = useNavigate()
  const [searchTerm, setSearchTerm] = useState('')
  const [clientes, setClientes] = useState<ClienteFound[]>([]) // ✅ ARRAY
  const [searched, setSearched] = useState(false)
  const [totalClientes, setTotalClientes] = useState(0)
  ...
  const handleViewCliente = (clienteId: string) => {
    navigate(`/admin/admin_clientes/${clienteId}`)
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          {/* Hint de búsqueda por teléfono */}
          <p className="text-xs text-muted-foreground mt-2">
            💡 Puedes buscar por teléfono con o sin prefijo (+593, 593, 0...)
          </p>
        </CardContent>
      </Card>
      ...
      {clientes.length > 0 && (
        <div className="space-y-4">
          ...
          {/* Grid de cards con resultados */}
          <div className="grid grid-cols-1 gap-4">
            {clientes.map((cliente) => (
              <BuscarClienteCard 
                key={cliente.user_id}
                cliente={cliente} 
                onViewDetails={() => handleViewCliente(cliente.user_id)}
              />
            ))}
          </div>
        </div>
      ...
      {/* Estado: No encontrado */}
      {searched && clientes.length === 0 && !loading && storeError && (
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-12 text-muted-foreground">
              <User className="h-16 w-16 mx-auto mb-4 opacity-50" />
              ...
              <p className="text-sm">
                Intenta buscar por documento, email, nombre o teléfono
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-12 text-muted-foreground">
              ...
              <h3 className="text-lg font-semibold mb-2">Busca un cliente</h3>
              <p className="text-sm">
                Ingresa el documento, email, nombre o teléfono del cliente que deseas encontrar
              </p>
            </div>
          </CardContent>
        </Card>
