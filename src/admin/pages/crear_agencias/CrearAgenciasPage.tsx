...
import { useState } from 'react'
import { useNavigate } from 'react-router' // ← Ya lo tienes importado
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
...
import { useAuthStore } from '@/stores/authStore'
import { useUserListStore } from '@/stores/userListStore' // ← NUEVO

export default function CrearAgenciasPage() {
  const { signUpAgenciaByAdmin, loading, error, user } = useAuthStore()
  const navigate = useNavigate() // ← Ya existe
  const { reset: resetUserList } = useUserListStore() // ← NUEVO: Para invalidar cache
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!currentError) {
        ...
        // ← NUEVO: Invalidar cache de agencias
        resetUserList()
        
        // ← NUEVO: Navegar después de 1.5 segundos
        setTimeout(() => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {success && (
            <Alert className="border-green-500 bg-green-50">
              <CheckCircle2 className="h-4 w-4 text-green-600" />
              <AlertDescription className="text-green-800">
                ✅ Agencia creada exitosamente. Redirigiendo a la lista...
              </AlertDescription>
            </Alert>
          )}
