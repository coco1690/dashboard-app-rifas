...
export const useVentaStore = create<VentaStore>((set, get) => ({
  ...
  venderBoletosPorSMS: async (data: VentaWhatsAppData): Promise<OrdenBoleto | null> => {
    ...
    const loadingToast = toast.loading('Procesando venta por SMS...', {
      description: `Vendiendo ${boletos.length} boleto${boletos.length > 1 ? 's' : ''}`
    })
    ...
    try {
      ...
      const { data: orden, error: ordenError } = await supabase
        ...
        .insert([{
          ...
          estado_pago: estadoPago,
          metodo_pago: metodoPago,
          enviado_email: false,      // ✅ NUEVO
          enviado_whatsapp: false,   // ✅ NUEVO
          enviado_sms: false         // ✅ NUEVO (se actualiza después)
        }])
        .select()
        .single()
      ...
      try {
        ...
        set({ errorSMS: smsError.message })

        throw new Error(`No se pudo enviar SMS. Venta cancelada. Detalle: ${smsError.message}`)
      }

      // ÉXITO
      const mensajeExito = `Venta por SMS realizada: ${boletos.length} boleto${boletos.length > 1 ? 's' : ''} vendido${boletos.length > 1 ? 's' : ''} a ${nombre} (${phone}). Total: $${totalEsperado.toLocaleString()}. ✅ SMS enviado exitosamente`
      ...
      const errorMessage = err.message || 'Error desconocido en la venta'
      console.error('❌ Error en venta por SMS:', err)
    ...
    try {
      ...
      const { data: orden, error: ordenError } = await supabase
        ...
        .insert([{
          ...
          metodo_pago: metodoPago,
          enviado_email: false,      // ✅ NUEVO
          enviado_whatsapp: false,   // ✅ NUEVO (se actualiza después)
      ...
      // Enviar WhatsApp vía Edge Function
      try {
        console.log('📱 FASE 2: Enviando WhatsApp vía Edge Function...')
